...
function updateHTML(data, commentsNums) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    // 将 '  <br>' 或者 '<br><br>' 处理成 <p></p>
    html = html.replace(/(?:^|  <br>|<br><br>)(.+?)(?=  <br>|<br><br>|$)/g, (match, content) => {
        ...
        ${data[i].resourceList
          .map((item, index) => {
            if (item.type.includes('image'))
              ...
              return `<div class="div-img w-full overflow-hidden hide-scrollbar hover:shadow-md"><audio src="${item.externalLink}" controls="controls" crossorigin="anonymous" preload="metadata" class="bb-audio rounded cursor-pointer"></audio></div>`
            else return ''
          })
          .join('\n')}
      </div>`
  ...
  <div class="bb-div">
    <div class="datatime">
      <div class="hy-avatar-block"><a href="${bbMemo.memos}" target="_blank" class="hy-astyle"><img
            src="${bbMemo.useravatar}" class="hy-avatar"></a></div>
      <div class="hy-intro">
        <div class="hy-name"><a href="${bbMemo.memos}" target="_blank" class="bb-id-name">${
      data[i].creatorName
    }</a></div>
...
async function getCommentsCount(urls) {
  let res = await twikoo.getCommentsCount({
    // envId: 'https://twikoo.qxxiao.eu.org/', // 环境 ID
    envId: bbMemo.envId, // 环境 ID
    urls: urls, // 文章链接列表
