...
async function getFirstList() {
  ...
  updateHTML(resdata) // 将 笔记数据转换合适的 html元素并插入到页面中
          ...
          .map((item, index) => {
            if (item.type.includes('image'))
              return `<div class="div-img w-full overflow-hidden hide-scrollbar hover:shadow-md"><img src="${
                item.externalLink
              }" alt="${'img-' + index}" class="bb-img cursor-pointer" loading="lazy" decoding="async"></div>`
            else if (item.type.includes('video'))
              return `<div div-img w-full overflow-hidden hide-scrollbar hover:shadow-md"><video src="${item.externalLink}" controls="controls" crossorigin="anonymous" preload="metadata" class="bb-video cursor-pointer"></video></div>`
            else return ''
          })
          .join('\n')}
      </div>`
